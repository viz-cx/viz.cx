...
export default async function FeedPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return <div className="flex flex-col gap-3">{items.length === 0 ? <p className="opacity-60">Follow some authors — their posts appear here.</p> : items.map(p => <PostCard key={String(p._id)} post={p} lang={lang} />)}</div>
}
