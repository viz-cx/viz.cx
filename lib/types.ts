...
// remote* are set only for comments that arrived over ActivityPub. For those,
// `author` holds the handle text (there is no local profile to link to).
export interface CommentDoc {
  id: string; postId: string; author: string; parentId: string | null; body: string
  createdAt: Date; deletedAt: Date | null
  remoteActor: string | null; remoteHandle: string | null; remoteObject: string | null
}
