...
export async function uniqueSlug(author: string, title: string): Promise<string> {
  const base = slugify(title)   // [a-z0-9-] only, so safe inside a regex
  const rows = await sql<{ slug: string }[]>`select slug from posts where author = ${author} and slug ~ ${`^${base}(-\\d+)?$`}`
