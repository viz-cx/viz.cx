export function markdown(str: string): string {
  ...
  str = highlight_links(str, true)
  return str
}

export function markdownTitle(str: string): string {
  const strikethrough_pattern = /\~\~(.*?)\~\~/gm
...
export function fast_str_replace(search: string, replace: string, str: string): string {
...
function markdown_decode(text: string, rewrite_block: any = undefined) {
  ...
  for (const i in text_arr) {
    ...
    if ("***" == el) {
      ...
      result += markdown_code(context as string)
      result += "</" + block + ">"
    }
    html += result + "\n"
  }
...
function html_safe_images(html: string) {
  const arr = html.match(/<img [^>]*src="[^"]*"[^>]*>/gm)
  for (const i in arr) {
    const img = arr[i as any]
    const img_src_pattern = /(.*)src=\"(.*?)\"(.*)/gm
    const src = img.replace(img_src_pattern, "$2")
    const safe_src = safe_image(src)
    let new_img = ""
    if (false === safe_src) {
      new_img = ""
    } else {
...
export function highlight_links(text: string, is_html: boolean): string {
  ...
  if (is_html) {
    const html_img_pattern = /<img(.[^>]*)>/gim
    const html_href_pattern = /<a (.[^>]*)>(.*)<\/a>/gim
    let html_arr = text.match(html_img_pattern)
    if (null != html_arr) {
      for (const i in html_arr) {
    ...
    for (const i in summary_html) {
      text = fast_str_replace(summary_html[i], "<REPLACE_HTML_" + i + ">", text)
    }
  }

  const summary_links: string[] = []
  let num = 0

  let summary_mnemonics: string[] = []
  let mnemonics_num = 0

  const mnemonics_pattern = /&#[a-z0-9\-\.]+/g
  const mnemonics_arr = text.match(mnemonics_pattern)
  if (null != mnemonics_arr) {
    for (const i in mnemonics_arr) {
  ...
  for (const i in summary_mnemonics) {
    text = fast_str_replace(
      summary_mnemonics[i],
      "<REPLACE_MNEMONIC_" + i + ">",
      text
  ...
  const account_pattern = /@[a-z0-9\-\.]+/g
  const account_links = text.match(account_pattern)
  if (null != account_links) {
    for (const i in account_links) {
  ...
  const viz_protocol_pattern = /viz\:\/\/[@a-z0-9\-\.\/]+/g
  const viz_protocol_links = text.match(viz_protocol_pattern)
  if (null != viz_protocol_links) {
    for (const i in viz_protocol_links) {
  ...
  const sia_protocol_pattern = /sia\:\/\/[A-Za-z0-9_\-\.\/]+/g
  const sia_protocol_links = text.match(sia_protocol_pattern)
  if (null != sia_protocol_links) {
    for (const i in sia_protocol_links) {
  ...
  const ipfs_protocol_pattern = /ipfs\:\/\/[A-Za-z0-9_\-\.\/]+/g
  const ipfs_protocol_links = text.match(ipfs_protocol_pattern)
  if (null != ipfs_protocol_links) {
    for (const i in ipfs_protocol_links) {
  ...
  const http_protocol_links = text.match(http_protocol_pattern)
  if (null != http_protocol_links) {
    for (const i in http_protocol_links) {
      summary_links[num] = http_protocol_links[i]
      num++
    }
  }

  const magnet_protocol_pattern =
    /((magnet):[\u0400-\u04FF\-A-Z0-9+\u0026\u2019@#\/%?=()~_|!:,.&]*[\u0400-\u04FF\-A-Z0-9+\u0026@#\/%=~()_|])/gi
  const magnet_protocol_links = text.match(magnet_protocol_pattern)
  if (null != magnet_protocol_links) {
    for (const i in magnet_protocol_links) {
      summary_links[num] = magnet_protocol_links[i]
      num++
    }
  }

  const hashtags_pattern = /(|\b)#([^:@#!.,?\r\n\t <>()\[\]]+)(|\b)/g
  const hashtags_links = text.match(hashtags_pattern)
  if (null != hashtags_links) {
    for (const i in hashtags_links) {
  ...
  const unique_links = array_unique(summary_links)
  unique_links.sort(sort_by_length_desc)

  for (const i in unique_links) {
    text = fast_str_replace(unique_links[i], "<REPLACE_LINK_" + i + ">", text)
  }

  for (const i in unique_links) {
    const change_text = unique_links[i]
...
function markdown_code(text: string): string {
  ...
  const italic_pattern = /\_\_(.[\s\S]*?)\_\_/gm
  text = text.replace(italic_pattern, "<i>$1</i>")
  const strikethrough_pattern = /\~\~(.[\s\S]*?)\~\~/gm
  text = text.replace(strikethrough_pattern, "<strike>$1</strike>")
  const inline_code_pattern = /\`(.*?)\`/gm
  text = text.replace(inline_code_pattern, "<code>$1</code>")

  const images_pattern = /\!\[(.*?)\]\((.*?)\)/gm
  text = text.replace(images_pattern, '<img src="$2" alt="$1"/>')

  const links_pattern = /\[(.*?)\]\((.*?)\)/gm
  const link_pattern = /\[(.*?)\]\((.*?)\)/m
  const links_arr = text.match(links_pattern)
  if (null !== links_arr) {
    if (typeof links_arr[0] != "undefined") {
      for (const i in links_arr) {
        const link = links_arr[i]
        const link_arr = link.match(link_pattern)
        if (null !== link_arr) {
...
function safe_image(link: string): string | false {
  if (0 == link.indexOf("https://")) return link
  if (0 == link.indexOf("ipfs://")) return ipfs_link(link.substring(7))
  return false
}
